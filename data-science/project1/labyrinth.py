...
class Labyrinth:
    ...
    def __getitem__(self, item) -> Field:
        ...
        if x >= self.x_size or x < 0:
            raise ValueError(f"coorinate x={x} is out of bounds=<0, {self.x_size - 1}>")
        if y >= self.y_size or y < 0:
            raise ValueError(f"coorinate y={y} is out of bounds=<0, {self.y_size - 1}>")
...
def make_labyrinth(x_size, y_size, obstacle_fill_percent=10, deterministic=True, seed=1234569):
    """
    ...
    return Labyrinth(x_size, y_size, obstacles, start, end)


def demo():
    l = make_labyrinth(10,10, 25)
    print(l)
