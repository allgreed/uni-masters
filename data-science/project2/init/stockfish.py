...
class Stockfish:
    ...
    def get_best_move(self, cp_tolerance: int = 5) -> Optional[str]:
        ...
        while True:
            ...
            if splitted_text[0] == "bestmove":
                if splitted_text[1] == "(none)":
                    break
                self.info = last_text
                best = splitted_text[1]
                break

            if splitted_text[0] == "info" and splitted_text[2] == str(self.depth) and len(splitted_text) >= 21:
                paths.append((splitted_text[21], int(splitted_text[9])))
            last_text = text

        try:
            reference = dict(paths)[best]
            return [p[0] for p in paths if abs(p[1]) >= abs(reference) - cp_tolerance]
        except KeyError:
            return (best,)
